...
Refactor update into subsystem methods called in a fixed order each frame.


## Why This Phaser Change Matters
- This Phaser task (Refactor update into subsystem methods called in a fixed order each frame.) targets the engine competency for today: Arcade Physics movement/collisions, camera behavior, and scene transitions..
...
- Check Phaser API signatures while implementing Refactor update into subsystem methods called in a fixed order each frame. to avoid incorrect assumptions about method behavior.
- Use Phaser examples as implementation references for Refactor update into subsystem methods called in a fixed order each frame., then adapt them to your module architecture.
