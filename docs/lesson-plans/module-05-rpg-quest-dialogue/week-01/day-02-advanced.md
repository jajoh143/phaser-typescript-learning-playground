...
Implement keyboard-selectable dialogue options with active highlight.


## Why This Phaser Change Matters
- This Phaser task (Implement keyboard-selectable dialogue options with active highlight.) targets the engine competency for today: UI composition with containers/text, interaction flow, and event-driven state updates..
...
- Check Phaser API signatures while implementing Implement keyboard-selectable dialogue options with active highlight. to avoid incorrect assumptions about method behavior.
- Use Phaser examples as implementation references for Implement keyboard-selectable dialogue options with active highlight., then adapt them to your module architecture.
