...
Implement a real reset button with hover/press states and pointer events.


## Why This Phaser Change Matters
- This Phaser task (Implement a real reset button with hover/press states and pointer events.) targets the engine competency for today: Input coordinate mapping, efficient rendering updates, containers, and animation feedback..
...
- Check Phaser API signatures while implementing Implement a real reset button with hover/press states and pointer events. to avoid incorrect assumptions about method behavior.
- Use Phaser examples as implementation references for Implement a real reset button with hover/press states and pointer events., then adapt them to your module architecture.
